<script>
  ...
  async function handleDelete(id, deletedTodo) {
    await fetch(`/api/todo/delete/${id}`, {
      method: 'DELETE'
    });

    data.dbTodos = data.dbTodos.filter((todo) => todo !== deletedTodo);
  }
</script>
  ...
  <div class="flex w-1/3 flex-col space-y-3">
    {#each todos as todo, idx}
      ...
      <form
        ...
        <form
          class="mr-4 rounded-lg border-2 p-1"
          action="?/checkTodo&id={todo.id}"
          method="POST">
          <input type="hidden" />
        ...
        <button
          on:click|preventDefault={async () => handleDelete(todo.id, todo)}
          >x</button>
      </form>
    {/each}
  </div>
